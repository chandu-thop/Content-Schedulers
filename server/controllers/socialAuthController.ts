...
export const generateAuthUrl = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const origin = queryOrigin || req.headers.origin || "http://localhost:5173";
        const redirectUrl = `${origin.replace(/\/$/, "")}/account`;
...
//Sync connected accounts from Zerio into MongoDB
//GET /api/auth/sync
    ...
    try {
        ...
        for (const zAccount of zernioAccounts) {
            ...
            const account = await Account.findOneAndUpdate({
                ...
                handle: zAccount.username || zAccount.name || zAccount.handle || "Unknow",
                status: "connected",
